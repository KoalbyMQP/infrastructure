...
netboot_cmdline_txt_nfsboot () {
	...
	NFS_CMDLINE="dwc_otg.lpm_enable=0 console=serial0,115200 console=tty1 root=/dev/nfs nfsroot=$STATIC_IP:$NFS_ROOT/$PI_Number,vers=3 rw ip=dhcp init=/bin/sh rootwait     "
	echo "$NFS_CMDLINE" > "$TFTP_ROOT/$PI_Number/cmdline.txt"
...
#sudo mkdir -p "$NFS_ROOT/$PI_Number/etc"
#sudo touch "$NFS_ROOT/$PI_Number/etc/hostname"
echo "rpi-nfs" | sudo tee "$NFS_ROOT/$PI_Number/etc/hostname"
